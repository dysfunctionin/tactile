...
const diag = await page.evaluate(() => {
  const allCells = [...document.querySelectorAll('[data-object-id="low-root-sheet"]')].map((e) =>
    e.getAttribute("data-cell-address"),
  );
  ...
  const virtual = [...document.querySelectorAll("[data-virtual-cell-address]")].map((e) =>
    e.getAttribute("data-virtual-cell-address"),
  );
  const scroller = document.querySelector("[data-sheet-scroll]");
  const scrollers = [...document.querySelectorAll("[data-sheet-scroll]")].map((s) => ({
    w: s.clientWidth,
    h: s.clientHeight,
    sw: s.scrollWidth,
    sh: s.scrollHeight,
  ...
  return {
    countAll: document.querySelectorAll("[data-object-id]").length,
    countForRoot: allCells.length,
    uniqSample: uniq.slice(0, 40),
    hasM9: !!m9,
    hasB3: !!b3,
    hasC2: !!c2,
    hasA1: !!a1,
    uniqHasB3: uniq.includes("B3"),
    uniqHasC2: uniq.includes("C2"),
    uniqHasM9: uniq.includes("M9"),
    scroller: scroller
      ? {
...
const diag2 = await page.evaluate(() => {
  ...
  return {
    ...
    hasB3: !!document.querySelector('[data-object-id="low-root-sheet"][data-cell-address="B3"]'),
    uniq: uniq.slice(0, 20),
    hasM9: uniq.includes("M9"),
  };
});
...
for (const [name, fn] of [
  ...
  ["add-column", (p) => scen.addColumnsAction(p, "low", 1)],
]) {
  console.log(`\n--- ${name} ---`);
  const t0 = Date.now();
  try {
