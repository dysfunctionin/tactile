...
if (!npmCli && !dryRun) {
  console.error("tests:all must be started through npm");
  process.exit(2);
}
...
async function runScript(script) {
  if (dryRun) {
    console.log(`tests:all ${runId} -> ${script}`);
    return 0;
  }
...
if (dryRun) {
  console.log(`\ntests:all dry run completed: ${runId}`);
  process.exit(0);
}
...
if (missingTypes.length) {
  console.error(`\ntests:all did not record: ${missingTypes.join(", ")}`);
  process.exit(1);
}

console.log(
  `\ntests:all completed: ${summary.totals.pass}/${summary.totals.total} passed, ` +
