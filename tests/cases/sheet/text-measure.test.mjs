...
scenario("column fit uses the widest cell once, without double padding", () => {
...
scenario("column fit measures each column independently", () => {
  const object = objectWithCells({
...
scenario("column fit uses the widest single line of a multi-line cell", () => {
  const object = objectWithCells({
    [cellId(0, 0)]: { value: "ab\nabcdefghij" },
  });
...
scenario("column fit honors per-cell font size", () => {
  const object = objectWithCells({
...
scenario("empty columns fit at least their header label", () => {
  assert.equal(naturalColumnWidth(objectWithCells({}), 0), 32); // "A" @ 10px
  assert.equal(naturalColumnWidth(objectWithCells({}), 51), 38); // "AZ" @ 10px
});

scenario("row fit is driven by the largest font for single-line rows", () => {
  const object = objectWithCells({
...
scenario("row fit accounts for wrapped content at the cell's column width", () => {
  const object = objectWithCells({
...
scenario("row fit accounts for explicit newlines", () => {
  const object = objectWithCells({
    [cellId(0, 0)]: { value: "a\nb\nc" },
  });
  assert.equal(
...
scenario("empty rows fit at the single-line baseline", () => {
  // 11.5 * 1.18 = 13.57 + 14 = 27.57 -> 28.
  assert.equal(naturalRowHeight(objectWithCells({}), 0), 28);
});

scenario("auto height stays null for plain single-line rows", () => {
...
scenario("auto height uses the cell's font size for wrapped line height", () => {
  const object = objectWithCells({
...
scenario("embedded cells measure their projected label plus the plugin icon", () => {
  const object = objectWithCells({
...
scenario("link cells reserve the link icon alongside their URL text", () => {
  const object = objectWithCells({
...
scenario("auto heights are a sparse map of only rows that wrap", () => {
  const object = objectWithCells({
