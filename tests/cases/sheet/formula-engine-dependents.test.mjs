...
scenario("engine re-evaluates a range formula when an inside cell changes", () => {
...
scenario("engine leaves a range formula untouched when an outside cell changes", () => {
  const engine = engineWithRanges();
...
scenario("multi-column ranges re-evaluate only on column-inside edits", () => {
  const engine = engineWithRanges();
...
scenario("wide (>1024 row) ranges stay exact: inside hits, outside misses", () => {
  const engine = engineWithRanges();
...
scenario("registry removal keeps subsequent range lookups exact", () => {
  const engine = engineWithRanges();
