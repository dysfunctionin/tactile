...
scenario("axis reordering keeps formula references on the moved axis", () => {
  const map = new Map([[0, 1], [1, 2], [2, 0]]);
  assert.equal(reorderFormulaForAxis("=A1+B3", "row", map), "=A2+B1");
});

scenario("axis deletion shifts references beyond the removed axis", () => {
  assert.equal(adjustFormulaForAxis("=A1+A2+A3", "row", 1, "delete"), "=A1+A2+A2");
});

scenario("groups, filters and conditional ranges track structural edits", () => {
  assert.deepEqual(
