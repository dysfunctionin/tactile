...
scenario("virtual ranges rebase in a bounded band instead of following every scroll pixel", () => {
...
scenario("horizontal virtual ranges keep a 65-column sheet inside its axis bounds", () => {
  const rows = Array.from({ length: 256 }, (_, index) => index);
...
scenario("rapid scroll look-ahead is directional and bounded", () => {
...
scenario("default-matrix render windows stay bounded at arbitrary offsets", () => {
  const { rows, columns, rowGeometry, columnGeometry, metrics } = fixtureGeometry();
...
scenario("virtual range math clamps malformed offsets and geometry mismatches", () => {
  const { rows, columns, rowGeometry, columnGeometry, metrics } = fixtureGeometry();
...
scenario("selection projection uses numeric bounds without normalizing each cell", () => {
...
scenario("conditional-format ranges compile once and preserve decimal sign values", () => {
...
scenario("empty cells stay sparse and embedded timers do not live in ordinary SheetCell", async () => {
