...
scenario("a snapshot shares cells maps instead of deep cloning them", () => {
...
scenario("mutating a captured cells map copies it and leaves the snapshot exact", () => {
  const workspace = workspaceWithCell("before");
...
scenario("a map that no snapshot references is mutated in place", () => {
  const workspace = workspaceWithCell("before");
  const cells = workspace.objects.sheet.cells;
  assert.equal(cellsForMutation(cells), cells);
});

scenario("a copied map is reused until the next snapshot captures it", () => {
  const workspace = workspaceWithCell("before");
...
scenario("deleting a cell after a snapshot does not affect the snapshot", () => {
  const workspace = workspaceWithCell("before");
  const snapshot = cloneHistoryWorkspace(workspace);
