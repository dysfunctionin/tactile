...
scenario("viewport ranges snap to fixed 64 by 64 chunks", () => {
...
scenario("movement within one chunk produces the same canonical request", () => {
...
scenario("viewport reads reuse canonical chunks and merge only requested cells", async () => {
...
scenario("sheet snapshot windows preserve cell records and refresh on revision changes", async () => {
  const sheet = {
    id: "sheet",
    type: "sheet",
    title: "Sheet",
...
scenario("sheet aggregates are lazy and reuse full 64-row summaries", async () => {
  let cellReads = 0;
...
scenario("aggregates remain one lazy global operation and structure invalidates chunks once", async () => {
  const { calls, store } = operationStore();
  const manager = new FixedDatasetChunkManager(store, { maxCacheBytes: 16 * 1024 * 1024 });
