...
scenario("window keys distinguish revisions, views, ranges, and column projections", () => {
  const base = {
    datasetId: "dataset",
    rowStart: 0,
    rowEnd: 99,
...
scenario("the cache evicts least-recently-used unpinned windows within its byte budget", () => {
...
scenario("pinned visible windows are not evicted and impossible inserts are rejected", () => {
...
scenario("a rejected insert does not partially evict existing windows", () => {
...
scenario("concurrent loads for one window share a single request", async () => {
...
scenario("revision invalidation removes matching windows even when pinned", () => {
  const cache = new DatasetWindowCache(100);
