...
scenario("golden v1-v4 fixtures migrate sequentially to the v4 contract", async () => {
...
scenario("the compatibility path preserves metadata that a legacy projection drops", async () => {
...
scenario("malformed, oversized, duplicate, and dangling fixtures fail safely", async () => {
...
scenario("newer unsupported formats are rejected without destructive normalization", async () => {
  const golden = await readFixture(4);
...
scenario("v4 portable package round-trips nested metadata and binary asset records", async () => {
  const fixture = await readFixture(4);
