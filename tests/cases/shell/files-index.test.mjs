...
scenario("files index exposes canonical and alias locations without changing Home ordering", () => {
...
scenario("files search matches titles and ignores case and accents", () => {
...
scenario("ordinary metadata edits reuse the cached topology graph", () => {
  const workspace = filesWorkspace();
  const index = buildFilesIndex(workspace);
  const changed = {
    ...workspace,
...
scenario("files deletion removes canonical descendants and aliases while protecting Home and Start", () => {
...
scenario("files rename validation trims names and rejects empty or duplicate titles", () => {
  const index = buildFilesIndex(filesWorkspace());
...
scenario("files distinguishes ordinary Tiles, the root Home Tiles, and the selected Start object", () => {
  const workspace = filesWorkspace();
  const index = buildFilesIndex(workspace);
...
scenario("files index carries object icon customizations without changing topology", () => {
  const workspace = filesWorkspace();
