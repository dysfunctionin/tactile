...
scenario("record commits persist cells without rewriting unrelated asset blobs", async () => {
...
scenario("asset URLs are reference-counted and revoked after the final release", () => {
  const created = [];
  const revoked = [];
...
scenario("browser persistence exposes session metadata and asset handles for native-compatible previews", async () => {
  const indexedDB = new MemoryIndexedDB();
  const localStorage = new MemoryStorage();
  const revoked = [];
...
scenario("legacy migration copies, verifies, switches, and retains the old store", async () => {
  const indexedDB = new MemoryIndexedDB();
  const localStorage = new MemoryStorage();
...
scenario("migration verification failure keeps the legacy record available", async () => {
  const indexedDB = new MemoryIndexedDB();
  const localStorage = new MemoryStorage();
