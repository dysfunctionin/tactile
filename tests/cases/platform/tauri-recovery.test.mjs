...
scenario("a malformed native acknowledgement can be rejected and recovered by the next commit", async () => {
  const calls = [];
...
scenario("closing and reopening a native workspace clears stale acknowledgement state and scope", async () => {
  const calls = [];
...
scenario("checkpoint rejects an older revision before it can cross the native boundary", async () => {
  const calls = [];
