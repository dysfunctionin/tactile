...
function memoryStorage(initialValue = null) {
  ...
  return {
    ...
    setItem: (key, next) => {
      if (key === CODE_RUNTIME_PROFILE_STORAGE_KEY) value = next;
    },
    value: () => value,
  };
}

scenario("code runtime profiles preserve only known non-empty tool paths", () => {
  assert.deepEqual(
    normalizeCodeRuntimeProfile({
      version: 99,
...
scenario("code runtime profile normalizes selection and cached discovery", () => {
  assert.deepEqual(
    normalizeCodeRuntimeProfile({
      paths: {},
...
scenario("code runtime profile store survives corrupt storage and publishes updates", () => {
