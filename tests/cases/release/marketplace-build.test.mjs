...
scenario("the generated catalog contains every independently compiled plugin", () => {
  assert.deepEqual(
...
scenario("catalog hashes and sizes match independently emitted artifacts", () => {
...
scenario("compiled plugins contain no unresolved host or cross-plugin imports", async () => {
  for (const entry of catalog.plugins) {
    const pluginFile = artifactFile(entry.artifact);
...
scenario("PDF owns its worker asset while other plugins remain independent", () => {
