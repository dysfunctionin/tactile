...
const scenario = defineSuite({ type: "e2e", suite: "in-out-layer-window" });
...
scenario("updates floating layer geometry when the browser viewport resizes", async ({ page }) => {
...
scenario("renders only the active parent and child during nested In & Out navigation", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("retains the mounted spatial layer slot across nested advance and return", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("opens an embedded tile with Enter just like the In & Out shortcut", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("does not retain listeners across 100 floating In & Out cycles", async ({ page }) => {
...
scenario("renders the opened tile sheet instead of an empty lazy surface", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("keeps the bottom dock available while a child floats", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("keeps the returning parent sheet fully populated while the child contracts", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("keeps the source tile corners rounded through the final collapse frame", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("uses the reverse expand curve when a full child collapses", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("keeps full-view chrome stable during the expanded-to-floating handoff", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("outside-clicking a floating layer closes exactly one history entry when titles repeat", async ({ page }) => {
  await page.goto("/");
...
scenario("changing home preserves the active parent chain and dock ordering", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("keeps one clean source cue while the child surface opens", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
