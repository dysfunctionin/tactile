...
const scenario = defineSuite({ type: "e2e", suite: "sheet-resize-preview" });
...
scenario("previews column resizing before the pointer is released and auto-fits on double-click", async ({ page }) => {
  await page.goto("/");
...
scenario("previews row resizing before the pointer is released and auto-fits on double-click", async ({ page }) => {
  await page.goto("/");
