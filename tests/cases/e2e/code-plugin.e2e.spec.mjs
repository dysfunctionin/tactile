...
const scenario = defineSuite({ type: "e2e", suite: "code-plugin" });

scenario("code plugin opens, applies its styles, and runs JavaScript", async ({ page }) => {
  const pageErrors = [];
...
scenario("HTML object can use an HTML Code cell as a live source", async ({ page }) => {
  await page.goto("/");
