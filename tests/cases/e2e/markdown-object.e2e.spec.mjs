...
const scenario = defineSuite({ type: "e2e", suite: "markdown-object" });
...
scenario("portaled Markdown color menus stay inside a floating child", async ({ page }) => {
  await page.goto("/");
...
scenario("markdown surfaces hide file metadata while linked editing and navigation still work", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("lazily renders accessible inline and display math without blocking edits", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("renders a Markdown document with visible LaTeX and Mermaid output", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("keeps themed Mermaid diagrams contained on a narrow viewport", async ({ page }) => {
...
scenario("loads Mermaid near the viewport and reuses isolated session renders", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("renders upstream Mermaid diagram types and localizes invalid source", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("continues Markdown lists intelligently when Enter is pressed", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("opens a Paper command menu for selected Markdown text", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
