...
scenario("files drawer searches and directly opens a nested object", async ({ page }) => {
  await page.goto("/");
...
scenario("files opens deep routes directly without replaying every ancestor transition", async ({ page }) => {
  await page.goto("/");
...
scenario("clicking the Files dock button again closes the temporary panel", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
  const filesButton = page.getByRole("button", { name: "Browse files", exact: true });
  await filesButton.click();
...
scenario("bottom-bar controls keep rounded corners and the Files hint uses readable text", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
...
scenario("hover tips render outside clipped panels", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
  await page.getByRole("button", { name: "Browse files", exact: true }).click();
...
scenario("files uses a custom context menu for workspace object actions", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
  await page.getByRole("button", { name: "Browse files", exact: true }).click();
...
scenario("files context-menu Delete removes the object link and protects Home and Start", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
  await page.getByRole("button", { name: "Browse files", exact: true }).click();
...
scenario("files context-menu Rename validates names and preserves live object links", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
  await page.getByRole("button", { name: "Browse files", exact: true }).click();
...
scenario("files lets users recolor a default icon or replace it with emoji", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
  await page.getByRole("button", { name: "Browse files", exact: true }).click();
...
scenario("files limits custom icons to one emoji grapheme and preserves presets and reset", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
  await page.getByRole("button", { name: "Browse files", exact: true }).click();
...
scenario("opening Files keeps the bottom dock above the workspace scrim", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
  await page.getByRole("button", { name: "Browse files", exact: true }).click();
...
scenario("embedded sheet icons use the linked object's Files color", async ({ page }) => {
  await page.goto("/");
  const workspace = filesWorkspace();
...
scenario("files can stay pinned as a sidebar while the workspace remains usable", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
  await page.getByRole("button", { name: "Browse files", exact: true }).click();
