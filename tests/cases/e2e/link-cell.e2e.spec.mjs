...
const scenario = defineSuite({ type: "e2e", suite: "link-cell" });
...
scenario("a bare-URL tile opens as a floating link window and materializes a link object", async ({ page }) => {
  await page.goto("/");
...
scenario("] opens the focused bare-URL tile as an embedded link", async ({ page }) => {
  await page.goto("/");
  await importWorkspace(page);
