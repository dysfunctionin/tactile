...
const scenario = defineSuite({ type: "e2e", suite: "arrow-navigation" });
...
scenario("walks one cell at a time and extends the range only with Shift", async ({ page }) => {
  await page.goto("/");
...
scenario("ctrl-click adds and removes individual tiles from the selection", async ({ page }) => {
  await page.goto("/");
...
scenario("keeps rapid arrow walking lossless and scrolls only to the active cell", async ({ page }) => {
  await page.goto("/");
