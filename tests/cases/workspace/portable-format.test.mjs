...
scenario("CSV codec preserves commas, quotes, and line breaks", () => {
  const rows = [["plain", "comma, value", 'a "quote"', "two\nlines"]];
  assert.deepEqual(parseCsv(stringifyCsv(rows)), rows);
});

scenario("tactile links remain human-readable and escape titles", () => {
...
scenario("sheet export writes only the used range", () => {
...
scenario("files preferences round-trip inside the portable workspace settings", async () => {
...
scenario("embedded objects serialize as links and round-trip from a bundle", async () => {
...
scenario("bare http(s) URLs are detected as link cells with host titles", () => {
...
scenario("a bare-URL cell materializes an embedded link object that round-trips from a bundle", async () => {
...
scenario("portable workspaces preserve the containment path to a nested home", async () => {
