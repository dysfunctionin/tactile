...
scenario("the installed app bundles only Tiles, Text, and internal Link support", () => {
...
scenario("marketplace catalog validation accepts schema v1 and rejects malformed data", async () => {
...
scenario("development uses local marketplace artifacts while production defaults to GitHub", () => {
...
scenario("alpha and RC builds use the alpha marketplace catalog while stable uses main", () => {
...
scenario("development activation replaces stale cached source without changing install state", async () => {
  const record = {
...
scenario("marketplace updates appear only for a newer catalog version", () => {
...
scenario("disabled installed plugins remain available in the Cell Objects list", () => {
...
scenario("plugin updates preserve the installed enablement state", () => {
...
scenario("a GitHub-hosted catalog resolves relative bundles and assets without recompiling Tactile", async () => {
...
scenario("plugin downloads verify size and SHA-256 before activation", async () => {
...
scenario("marketplace install size and progress include the bundle and declared assets", async () => {
...
scenario("plugin package assets are verified and returned for persistent caching", async () => {
  const source = "export function activate() { return {}; }";
...
scenario("portable workspaces preserve opaque plugin state and requirements without the plugin", async () => {
