...
scenario("parses built-in Markdown formatting without interpreting protected math", () => {
...
scenario("supports dollar and TeX inline delimiters while preserving escapes and currency", () => {
...
scenario("parses multiline display math and leaves unmatched delimiters as text", () => {
...
scenario("classifies only exact Mermaid fences as diagrams and never parses their source", () => {
...
scenario("uses strict Mermaid configuration and theme-aware normalized cache keys", async () => {
  const light = {
    colorScheme: "light",
    paper: "#ffffff",
